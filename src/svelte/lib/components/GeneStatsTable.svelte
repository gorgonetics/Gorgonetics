<script>
  ...
  function calculateGrandTotal() {
    ...
    if (currentView === "attribute") {
      const attributes = [
        "Intelligence", "Toughness", "Friendliness", "Ruggedness",
        "Ferocity", "Enthusiasm", "Virility", "Speed"
      ];
...
<div class="stats-section">
  <div class="stats-table-container">
    <div class="table-header">
      <h4 id="tableTitle">
        {currentView === "attribute" ? "Attribute Effects Summary" : "Appearance Effects Summary"}
      </h4>
    ...
    <div class="table-instructions">
      Click attributes to select • Ctrl+click to add multiple • Alt+click to hide
    </div>

    <table class="stats-table">
      <thead id="tableHeaders">
        <tr>
          <th>{currentView === "attribute" ? "Attribute" : "Effect Type"}</th>
          <th>Count</th>
          <th>Details</th>
        </tr>
      </thead>
      <tbody id="tableBody">
        {#if currentView === "attribute"}
          {#each [
            ...
            <tr
              class="attribute-row"
              class:selected={isAttributeSelected(attr.key)}
              class:hidden-attribute={isAttributeHidden(attr.key)}
        ...
        {:else}
          {#each [
            { key: "body-color-hue", name: "Body Color Hue", examples: "Color tone" },
            { key: "body-color-saturation", name: "Body Color Saturation", examples: "Color intensity" },
            { key: "body-color-intensity", name: "Body Color Intensity", examples: "Brightness" },
            { key: "wing-color-hue", name: "Wing Color Hue", examples: "Wing tone" },
            { key: "wing-color-saturation", name: "Wing Color Saturation", examples: "Wing intensity" },
            { key: "wing-color-intensity", name: "Wing Color Intensity", examples: "Wing brightness" },
            ...
            { key: "antenna-scale", name: "Antenna Scale", examples: "Antenna size" },
            { key: "leg-deformity", name: "Leg Deformity", examples: "Leg shape" },
            { key: "antenna-deformity", name: "Antenna Deformity", examples: "Antenna shape" },
            { key: "particles", name: "Particles", examples: "Special effects" },
            { key: "particle-location", name: "Particle Location", examples: "Effect position" },
            { key: "glow", name: "Glow", examples: "Luminescence" }
          ] as type}
            {@const count = currentStats?.[type.key] || 0}
            <tr
              class="appearance-row"
              class:selected={isAttributeSelected(type.key)}
              class:hidden-attribute={isAttributeHidden(type.key)}
    ...
    <div class="summary-info">
      <span>Total Genes: <span id="totalGenesDisplay">{totalGenes}</span></span>
