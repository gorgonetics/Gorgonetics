<script>
  ...
  function isGeneChanged(gene) {
    ...
    return (
      (gene.effect_dominant || 'None') !== (original.effect_dominant || 'None') ||
      (gene.effect_recessive || 'None') !== (original.effect_recessive || 'None') ||
  ...
  function getAttributeClass(effect) {
    if (!effect || effect === 'None') return '';
    if (effect.includes('Intelligence')) return 'option-intelligence';
    if (effect.includes('Toughness')) return 'option-toughness';
    if (effect.includes('Speed')) return 'option-speed';
    if (effect.includes('Friendliness')) return 'option-friendliness';
    if (effect.includes('Ruggedness')) return 'option-ruggedness';
    if (effect.includes('Ferocity')) return 'option-ferocity';
    if (effect.includes('Enthusiasm')) return 'option-enthusiasm';
    if (effect.includes('Virility')) return 'option-virility';
    return '';
  }

  function getAttributeEmoji(effect) {
    if (!effect || effect === 'None') return '';
    if (effect.includes('Intelligence')) return '🧠';
    if (effect.includes('Toughness')) return '💪';
    if (effect.includes('Speed')) return '⚡';
    if (effect.includes('Friendliness')) return '😊';
    if (effect.includes('Ruggedness')) return '🛡️';
    if (effect.includes('Ferocity')) return '😤';
    if (effect.includes('Enthusiasm')) return '🎉';
    if (effect.includes('Virility')) return '💖';
    return '';
  }
...
<div class="gene-editing-view">
  <div class="genes-header">
    ...
    <div class="genes-stats">
      ...
      <span class="stat-item">{Object.keys(geneBlocks).length} blocks</span>
    ...
    >
      {savingChanges ? 'Saving...' : hasUnsavedChanges ? 'Save Changes' : 'No Changes'}
    </button>
  </div>
    ...
    <div class="genes-content">
      {#each Object.entries(geneBlocks) as [blockPrefix, block]}
          <div class="gene-block" class:collapsed={block.collapsed}>
            <div class="gene-block-header" on:click={() => toggleGeneBlock(blockPrefix)}>
              <span class="fold-icon">{block.collapsed ? '▶' : '▼'}</span>
              <span class="block-title">Block {blockPrefix}</span>
              <span class="block-count">({block.genes.length} genes)</span>
            </div>

            {#if !block.collapsed}
              <div class="gene-row">
                {#each block.genes as gene}
                  <div class="gene-card" class:has-changes={isGeneChanged(gene)}>
                    ...
                    <div class="gene-fields">
                      <div class="gene-field">
                        ...
                        <div class="custom-select-wrapper">
                          <div
                            class="custom-select-selected {getEffectClass(gene.effect_dominant)}"
                            on:click={() => toggleDropdown(`${gene.gene}-dominant`)}
                          >
                          ...
                          {#if openDropdown === `${gene.gene}-dominant`}
                            <div class="custom-select-options">
                              <div
                                ...
                                on:click={() => selectOption(gene, 'effect_dominant', 'None')}
                              >
                                None
                              </div>
                              <div class="options-grid">
                                <div class="negative-column">
                                  {#each effectOptions.sort().filter(option => option.includes('-')) as option}
                                    <div
                                      class="custom-option option-negative"
                                      on:click={() => selectOption(gene, 'effect_dominant', option)}
                                    >
                                      {option}
                                    </div>
                                  {/each}
                      ...
                      <div class="gene-field">
                        ...
                        <div class="custom-select-wrapper">
                          <div
                            class="custom-select-selected {getEffectClass(gene.effect_recessive)}"
                            on:click={() => toggleDropdown(`${gene.gene}-recessive`)}
                          >
                          ...
                          {#if openDropdown === `${gene.gene}-recessive`}
                            <div class="custom-select-options">
                              <div
                                class="custom-option option-none none-option"
                                on:click={() => selectOption(gene, 'effect_recessive', 'None')}
                              >
                                None
                              </div>
                              <div class="options-grid">
                                <div class="negative-column">
                                  {#each effectOptions.sort().filter(option => option.includes('-')) as option}
                                    <div
                                      class="custom-option option-negative"
                                      on:click={() => selectOption(gene, 'effect_recessive', option)}
                                    >
                                      {option}
                                    </div>
                                  {/each}
                      ...
                      <div class="gene-field">
                        ...
                        <input
                          ...
                          on:input={(e) => handleInputChange(gene, 'appearance', e.target.value)}
                        />
                      </div>
                      ...
                      {#if expandedNotes[gene.gene]}
                        <div class="notes-content expanded">
                          <textarea
                            ...
                            value={gene.notes || ''}
                            on:input={(e) => handleInputChange(gene, 'notes', e.target.value)}
                          ></textarea>
                        </div>
                      {/if}
                    </div>
...
<style>
  ...
  /* Scrollbar styling */
  .gene-field select::-webkit-scrollbar {
    width: 8px;
  }

  .gene-field select::-webkit-scrollbar-track {
    background: #f1f1f1;
    border-radius: 4px;
  }

  .gene-field select::-webkit-scrollbar-thumb {
    background: #c1c1c1;
    border-radius: 4px;
  }

  .gene-field select::-webkit-scrollbar-thumb:hover {
    background: #a8a8a8;
  }
