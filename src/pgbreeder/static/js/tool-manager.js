...
class ToolManager {
  ...
  renderPetsList(pets) {
    ...
    if (pets.length === 0) {
      petsList.innerHTML =
        '<div class="empty-state">No pets in collection</div>';
      return;
    }
  ...
  renderPetsStats(pets) {
    ...
    const speciesStats = Object.entries(species)
      .map(
        ([name, count]) =>
          `<div class="stat-item"><strong>${count}</strong> ${name}</div>`,
      )
      .join("");
  ...
  selectPet(petId) {
    ...
    console.log(
      "Current edit form element:",
      document.getElementById("petEditForm"),
    );
  ...
  async savePetChanges() {
    ...
    const petUpdate = {
      ...
      intelligence:
        parseFloat(document.getElementById("editIntelligence").value) || null,
      toughness:
        parseFloat(document.getElementById("editToughness").value) || null,
      friendliness:
        parseFloat(document.getElementById("editFriendliness").value) || null,
      ruggedness:
        parseFloat(document.getElementById("editRuggedness").value) || null,
      ferocity:
        parseFloat(document.getElementById("editFerocity").value) || null,
      enthusiasm:
        parseFloat(document.getElementById("editEnthusiasm").value) || null,
      virility:
        parseFloat(document.getElementById("editVirility").value) || null,
