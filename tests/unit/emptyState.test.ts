...
describe('EmptyState', () => {
  it('renders the title', () => {
    ...
    expect(container.querySelector('.status-title')?.textContent).toBe('Pick a pet');
  });

  it('renders the body when provided', () => {
    const { container } = mount({ body: 'Choose one from the list.' });
    expect(container.querySelector('.status-body')?.textContent).toBe('Choose one from the list.');
  });

  it('omits the body element when not provided', () => {
    const { container } = mount();
    expect(container.querySelector('.status-body')).toBeNull();
  });

  it('renders the icon when provided', () => {
    const { container } = mount({ icon: '🐾' });
    expect(container.querySelector('[data-testid="status-pane-icon"]')?.textContent).toBe('🐾');
  });

  it('omits the icon when not provided', () => {
    const { container } = mount();
    expect(container.querySelector('[data-testid="status-pane-icon"]')).toBeNull();
  });
  ...
  it('shows the action and fires onAction only when both label and handler are given', async () => {
    ...
    const btn = container.querySelector('[data-testid="status-pane-action"]') as HTMLButtonElement;
  ...
  it('hides the action when the label is missing even if a handler is passed', () => {
    ...
    expect(container.querySelector('[data-testid="status-pane-action"]')).toBeNull();
  });
