...
// `id`/`name` fields the assertions read, so cast the partial literal.
const asPet = (p: { id: number; name: string }) => p as unknown as Pet;
...
describe('Pets Store', () => {
  ...
  describe('selectPet', () => {
    it('sets the selected pet', () => {
      const fakePet = asPet({ id: 1, name: 'TestBee' });
  ...
  describe('appendPet', () => {
    ...
    it('survives a slower in-flight loadPets that snapshotted before the append', async () => {
      ...
      let resolveStale: (value: { items: Pet[]; total: number }) => void;
      ...
      resolveStale!({ items: [] } as unknown as { items: Pet[]; total: number });
      await stalePromise;
