...
type GeneRow = Record<string, string | number | null>;
...
describe('upsertGene persists parsed columns', () => {
  ...
  it('writes dominant_attribute/sign and recessive_attribute/sign', async () => {
    ...
    const db = getDb();
    const rows = await db.select<GeneRow[]>(
...
describe('backfillParsedGeneEffectsIfNeeded', () => {
  ...
  async function insertLegacyGene(gene: string, effectDominant: string, effectRecessive: string) {
  ...
  it('steady-state run is a no-op over already-parsed rows', async () => {
    ...
    await geneService.backfillParsedGeneEffectsIfNeeded();
    const db = getDb();
    const [before] = await db.select<GeneRow[]>('SELECT updated_at FROM genes WHERE animal_type = $at AND gene = $g', {
      at: 'beewasp',
      g: '01A1',
    });
    ...
    await geneService.backfillParsedGeneEffectsIfNeeded();
    const [after] = await db.select<GeneRow[]>('SELECT updated_at FROM genes WHERE animal_type = $at AND gene = $g', {
      at: 'beewasp',
      g: '01A1',
    });
