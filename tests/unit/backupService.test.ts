...
async function buildZip({ metaOverrides = {}, genes = null, pets = null }: BuildZipOptions = {}) {
  const zip = new JSZip();
  const metadata = {
...
describe('Backup Service', () => {
  ...
  describe('loadBackup', () => {
    ...
    it('importDatabase re-validates metadata even on a pre-loaded backup', async () => {
      ...
      const tampered = {
        zip: loaded.zip,
        metadata: { ...loaded.metadata, format: 'other-app' },
      } as unknown as LoadedBackup;
      await expect(
  ...
  describe('importDatabase — genes', () => {
    const importOpts = (mode: ImportMode) => ({
      mode,
      includeGenes: true,
      includePets: false,
      includeImages: false,
    ...
    it('imports genes in replace mode', async () => {
      ...
      const db = getDb();
      const rows = await db.select<{ gene: string }[]>('SELECT * FROM genes');
    ...
    it('replaces existing genes on re-import', async () => {
      ...
      const db = getDb();
      const rows = await db.select<{ effectDominant: string }[]>('SELECT * FROM genes');
      expect(rows).toHaveLength(1);
  ...
  describe('importDatabase — pets', () => {
    ...
    it('imports pets in replace mode', async () => {
      ...
      const db = getDb();
      const rows = await db.select<{ name: string }[]>('SELECT * FROM pets');
      expect(rows).toHaveLength(1);
    ...
    it('offsets sort_order in merge mode', async () => {
      ...
      const db = getDb();
      const rows = await db.select<{ name: string; sort_order: number }[]>(
        'SELECT name, sort_order FROM pets ORDER BY sort_order',
      );
    ...
    it('wipes the stale ledger and re-seeds it with restored pet hashes on replace', async () => {
      ...
      const ledgerBefore = await db.select<{ content_hash: string }[]>('SELECT content_hash FROM imported_files');
      ...
      const ledgerAfter = await db.select<{ content_hash: string; source_path: string }[]>(
    ...
    it('preserves existing ledger AND adds restored pet hashes on merge', async () => {
      ...
      const ledger = await db.select<{ content_hash: string }[]>(
        'SELECT content_hash FROM imported_files ORDER BY content_hash',
      );
    ...
    it('coerces missing genome_text to "" for pre-v13 backups', async () => {
      ...
      const legacyPet: Record<string, unknown> = { ...samplePet };
      delete legacyPet.genome_text;
      const zipData = await buildZip({ pets: [legacyPet] });
      const result = await importDatabase(zipData, importOpts('replace'));
      expect(result.pets).toBe(1);

      const rows = await getDb().select<{ genome_text: string }[]>(
        'SELECT genome_text FROM pets WHERE content_hash = $h',
        { h: 'hash_abc' },
      );
  ...
  describe('importDatabase — combined', () => {
    ...
    it('replace re-import keeps count stable', async () => {
      ...
      const db = getDb();
      const before = await db.select<{ count: number }[]>('SELECT COUNT(*) as count FROM pets');
      ...
      const after = await db.select<{ count: number }[]>('SELECT COUNT(*) as count FROM pets');
