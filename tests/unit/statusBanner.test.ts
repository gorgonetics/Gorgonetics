...
// StatusBanner declares its props via an untyped `$props()`, so the
// inferred props treat every field as required. The banner's
// `autoDismissMs` / `onDismiss` are optional in practice; cast each
// partial props literal to the full props type to satisfy the checker
// without changing what is passed at runtime.
type BannerProps = ComponentProps<typeof StatusBanner>;
...
describe('StatusBanner', () => {
  ...
  it('renders the message and applies the variant class', () => {
    const { container } = render(StatusBanner, { type: 'success', message: 'Hello' } as unknown as BannerProps);
  ...
  it('uses role="alert" for errors and role="status" otherwise', async () => {
    const { container, rerender } = render(StatusBanner, { type: 'error', message: 'oops' } as unknown as BannerProps);
  ...
  it('does NOT auto-dismiss when autoDismissMs is missing', () => {
    const onDismiss = vi.fn();
    render(StatusBanner, { type: 'success', message: 'A', onDismiss } as unknown as BannerProps);
