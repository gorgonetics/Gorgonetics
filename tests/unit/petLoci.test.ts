...
import { Gender, type GeneType } from '$lib/types/index.js';
import { groupLociByChromosome, loadAllPetLoci, type PetLoci, walkPairLoci } from '$lib/utils/petLoci.js';
...
const MINIMAL_GENOME = (name: string, alleles: string) => `[Overview]
Format=1.0
Character=Tester
Entity=${name}
Genome=BeeWasp
...
async function uploadPet(name: string, alleles: string): Promise<number> {
...
describe('walkPairLoci', () => {
  function locus(entries: ReadonlyArray<[string, GeneType]>): PetLoci {
  ...
  it('calls the callback once per locus when both maps share keys', () => {
    ...
    const seen: Array<[string, GeneType, GeneType]> = [];
...
describe('groupLociByChromosome', () => {
  ...
  it('returns each locus with id, type, block, and position metadata', () => {
    const loci: PetLoci = new Map([['01A3', 'x']]);
    const grouped = groupLociByChromosome(loci);
