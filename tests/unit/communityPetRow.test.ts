...
vi.mock('$lib/stores/community.svelte.js', () => ({
  selectPet: (hash: string) => selectPetSpy(hash),
}));
...
describe('CommunityPetRow accessibility', () => {
  it('renders a real <tr> with grid row semantics (not button)', () => {
    ...
    const row = container.querySelector<HTMLElement>('[data-testid="community-row"]');
  ...
  it('exposes aria-selected reflecting the selected prop', async () => {
    ...
    let row = container.querySelector<HTMLElement>('[data-testid="community-row"]');
    ...
    row = container.querySelector<HTMLElement>('[data-testid="community-row"]');
