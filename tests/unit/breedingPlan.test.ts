...
describe('buildBatches', () => {
  it('returns an empty plan when spots <= 0', () => {
    const plan = buildBatches([pair(1, 2)], 0);
  ...
  it('takes the top-ranked disjoint pairs, skipping any that reuse an animal', () => {
    ...
    const { batches, matchedIds } = buildBatches(ranked, 4);
    expect(batches).toHaveLength(1);
    expect(batches[0].map((p) => [p.male.id, p.female.id])).toEqual([
      [1, 2],
      [5, 6],
    ]);
    expect([...matchedIds].sort((a, b) => a - b)).toEqual([1, 2, 5, 6]);
  });
  ...
  it('does not reuse an animal across batches (global disjointness)', () => {
    ...
    const { batches } = buildBatches(ranked, 1);
