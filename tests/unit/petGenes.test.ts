...
describe('pet_genes is populated on upload', () => {
  ...
  it('inserts one pet_genes row per genome position', async () => {
    ...
    const rows = await db.select<{ gene_id: string; gene_type: string }[]>(
      'SELECT gene_id, gene_type FROM pet_genes WHERE pet_id = $pid ORDER BY gene_id',
      {
        pid: result.pet_id,
      },
    );
  ...
  it('handles a realistic sample genome', async () => {
    ...
    const db = getDb();
    const [count] = await db.select<{ n: number }[]>('SELECT COUNT(*) as n FROM pet_genes WHERE pet_id = $pid', {
      pid: result.pet_id,
    });
...
describe('updatePet rewrites pet_genes when the genome changes', () => {
  ...
  it('replaces rows when genome_data is updated', async () => {
    ...
    await petService.updatePet(result.pet_id as number, { genome_data: emptyGenomeJson });
  ...
  it('does not touch pet_genes when only non-genome fields change', async () => {
    const result = await petService.uploadPet(MINIMAL_BEEWASP_GENOME, { name: 'Minimal', gender: 'Female' });
    const db = getDb();
    const [before] = await db.select<{ n: number }[]>('SELECT COUNT(*) as n FROM pet_genes WHERE pet_id = $pid', {
      pid: result.pet_id,
    });
    expect(before.n).toBe(3);

    await petService.updatePet(result.pet_id as number, { name: 'Renamed' });
    const [after] = await db.select<{ n: number }[]>('SELECT COUNT(*) as n FROM pet_genes WHERE pet_id = $pid', {
      pid: result.pet_id,
    });
...
describe('deletePet removes a pet and its pet_genes rows', () => {
  ...
  it('drops the pet and its pet_genes rows together', async () => {
    const result = await petService.uploadPet(MINIMAL_BEEWASP_GENOME, { name: 'Minimal', gender: 'Female' });
    await petService.deletePet(result.pet_id as number);
    const db = getDb();
    const [count] = await db.select<{ n: number }[]>('SELECT COUNT(*) as n FROM pet_genes WHERE pet_id = $pid', {
      pid: result.pet_id,
    });
...
describe('backfillPetGenesIfNeeded', () => {
  ...
  it('handles an empty pets table without error', async () => {
    await petService.backfillPetGenesIfNeeded();
    const db = getDb();
    const [count] = await db.select<{ n: number }[]>('SELECT COUNT(*) as n FROM pet_genes');
    expect(count.n).toBe(0);
  });
});
