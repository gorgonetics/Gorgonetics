...
describe('Database', () => {
  ...
  it('selects inserted genes', async () => {
    ...
    const rows = await db.select<Array<{ gene: string; effectDominant: string }>>(
      'SELECT * FROM genes WHERE animal_type = $animal_type',
      {
        animal_type: 'beewasp',
      },
    );
  ...
  it('selects distinct animal types', async () => {
    ...
    const rows = await db.select<Array<{ animal_type: string }>>(
  ...
  it('counts rows', async () => {
    ...
    const rows = await db.select<Array<{ cnt: number }>>('SELECT COUNT(*) as cnt FROM pets');
  ...
  it('updates rows', async () => {
    ...
    const rows = await db.select<Array<{ effectDominant: string }>>(
      'SELECT effectDominant FROM genes WHERE gene = $gene',
      { gene: '01A1' },
    );
    expect(rows[0].effectDominant).toBe('Toughness+');
  });
  ...
  describe('ORDER BY', () => {
    ...
    function petParams(name: string, hash: string, toughness = 50) {
      return {
        name,
        species: 'BeeWasp',
        gender: 'Male',
    ...
    it('sorts ascending by default', async () => {
      ...
      const rows = await db.select<Array<{ name: string }>>('SELECT * FROM pets ORDER BY name');
    ...
    it('sorts descending with DESC', async () => {
      ...
      const rows = await db.select<Array<{ name: string }>>('SELECT * FROM pets ORDER BY name DESC');
    ...
    it('sorts numerically for number columns', async () => {
      ...
      const rows = await db.select<Array<{ toughness: number }>>('SELECT * FROM pets ORDER BY toughness');
    ...
    it('supports multi-column ORDER BY', async () => {
      ...
      const rows = await db.select<Array<{ name: string }>>('SELECT * FROM pets ORDER BY toughness, name');
    ...
    it('supports mixed ASC/DESC in multi-column ORDER BY', async () => {
      ...
      const rows = await db.select<Array<{ name: string }>>('SELECT * FROM pets ORDER BY toughness, name DESC');
      expect(rows.map((r) => r.name)).toEqual(['Charlie', 'Bob', 'Alice']);
    });
  });
  ...
  it('deletes rows', async () => {
    ...
    const before = await db.select<Array<{ cnt: number }>>('SELECT COUNT(*) as cnt FROM pets');
    expect(before[0].cnt).toBe(1);

    const result = await db.execute('DELETE FROM pets WHERE name = $name', { name: 'ToDelete' });
    expect(result.rowsAffected).toBe(1);

    const after = await db.select<Array<{ cnt: number }>>('SELECT COUNT(*) as cnt FROM pets');
