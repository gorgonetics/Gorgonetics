...
function key(k: string) {
  return { key: k, preventDefault: vi.fn(), stopPropagation: vi.fn() } as unknown as KeyboardEvent & {
...
describe('createKeyboardReorder', () => {
  let items: string[];
  let persist: Mock<() => void | Promise<void>>;
  let announce: Mock<(message: string) => void>;
  let focusItem: Mock<(index: number) => void>;
  let kb: ReturnType<typeof createKeyboardReorder>;

  beforeEach(() => {
    items = ['Alpha', 'Bravo', 'Charlie'];
    persist = vi.fn<() => void | Promise<void>>().mockResolvedValue(undefined);
    announce = vi.fn<(message: string) => void>();
    focusItem = vi.fn<(index: number) => void>();
