...
type ImportResult = { genes: number; pets: number; petsSkipped: number };
type ImportOptions = {
  mode: 'replace' | 'merge';
...
async function createBackupZip(page: Page, { includeGenes = true, includePets = true } = {}) {
  return page.evaluate(
    async ({ includeGenes, includePets }) => {
      const { default: JSZip } = await import(/* @vite-ignore */ '/src/lib/services/jszip-reexport.ts' as string);
      const { getDb } = await import(/* @vite-ignore */ '/src/lib/services/database.ts' as string);
      const { CURRENT_SCHEMA_VERSION } = await import(
        /* @vite-ignore */ '/src/lib/services/migrationService.ts' as string
      );
      const db = getDb();
      const zip = new JSZip();
      ...
      if (includePets) {
        ...
        const petsExport = pets.map((p: Record<string, unknown>) => {
          const c = { ...p };
          delete c.id;
          return c;
        });
...
async function importBackupZip(page: Page, zipBase64: string, options: ImportOptions): Promise<ImportResult> {
  return page.evaluate(
    async ({ zipBase64, options }) => {
      const { importDatabase } = await import(/* @vite-ignore */ '/src/lib/services/backupService.ts' as string);
...
async function deleteFirstPet(page: Page) {
...
async function getPetState(page: Page) {
  return page.evaluate(async () => {
    const { getDb } = await import(/* @vite-ignore */ '/src/lib/services/database.ts' as string);
    const db = getDb();
    const pets = await db.select('SELECT * FROM pets ORDER BY name');
    ...
    return {
      ...
      geneCount: genes.length,
      petNames: pets.map((p: Record<string, unknown>) => p.name).sort(),
    };
  });
}
...
test.describe('Database Backup – v2 Zip', () => {
  ...
  test('database has correct schema version', async ({ page }) => {
    const { version, expected } = await page.evaluate(async () => {
      const { getSchemaVersion, CURRENT_SCHEMA_VERSION } = await import(
        /* @vite-ignore */ '/src/lib/services/migrationService.ts' as string
      );
