...
- **`Unknown` (`?`) is missing data, never a value.** In Project Gorgon a locus reads `?` because the owner's Genetics level had not revealed it **at the time that pet was studied**, and higher-level breeds' genes reveal later. Counting `?` would make late-reveal loci look artificially rare. So `?` contributes **no alleles and no denominator**, and a pet's own `?` cell has no rarity signal. See the `project-genetics-skill-unknown-genes` note.
...
- **Minimum sample.** A locus with fewer than `minKnown` **alleles** has no meaningful rare/common distinction. Default **4** (= 2 pets with a known reading); such loci render as "missing data", same as `?`. `minKnown` only suppresses the *degenerate* case; it is **not** where the ramp becomes informative.
  ...
  **The denominator always varies per locus — in every tier, including local ones.** A pet's genome records what was visible *when that pet was studied*, so a collection accumulated across a rising Genetics level holds pets revealed to different depths. Two pets in the same stable can disagree about which loci are known. (A fully-revealed collection means the owner re-studied everything after maxing the skill — an action, not a default.) So `minKnown` is live from day one, per-locus known counts must be carried through to the UI (§6), and the "same loci unknown across the whole collection" simplification does **not** hold. Re-studying pets is the player's lever for improving their own baseline. With self-inclusion, a pure pet contributes 2 of its locus's alleles, so at 2 pets its own allele is ≥50% → always "common". The ramp gets meaningful around ~5+ pets of a species (see §7) — sooner than genotype counting allowed, because the allele denominator is twice as large and a singleton allele reads at half the frequency.
...
- **Uneven study depth within one local population** — the case my earlier draft wrongly assumed away. A stable where some pets were studied at a lower Genetics level must yield **different `knownPets` at different loci**; assert the tooltip quotes the per-locus count rather than the population size, and that a locus dropping below `minKnown` renders as missing data while its neighbours still shade normally.
