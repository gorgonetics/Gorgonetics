...
- **`Unknown` (`?`) is missing data, never a value.** In Project Gorgon a locus reads `?` only because the player's Genetics skill hasn't revealed it yet — uniformly across the whole collection, and biased toward loci that unlock at the highest skill levels. Counting `?` would make late-unlock loci look artificially rare. So `?` contributes **no alleles and no denominator**, and a pet's own `?` cell has no rarity signal. See the `project-genetics-skill-unknown-genes` note.
  ...
  **Measured against the community catalogue** (52 shared Horse genomes): 51 are fully revealed and 1 is missing 522 of 1576 loci (33%). Within that genome the unknowns are **fine-grained, not banded** — 34 of 48 chromosomes are partially revealed, interleaved gene by gene (e.g. chr06 reads `?x?D xDRD D?DR DDRx ??D? …`). Only 5 chromosomes are wholly unknown and 9 wholly known.

  The one strong correlate is the **breed tag**, not the chromosome: Calico 100% unknown, Statehelm 94.7%, Leopard 57.6%, untagged loci 48.0%, then Ilmarian 30.3% and every remaining breed under 7% (Kurbone 2.3%, Standardbred 1.5%). The apparent chromosome clustering is entirely downstream of gene layout — each breed occupies exactly four dedicated chromosomes (Calico = chr45–48, Statehelm = chr29–32, Kurbone = chr13–16; untagged spans chr01–08), so "chr47 is 100% unknown" is just a restatement of "Calico is 100% unknown". **Why reveal varies by breed is not established here and is not assumed by this design.**

  What matters for v1 is unchanged and now quantified: a third of a genome can be missing, concentrated on some breeds' loci and scattered gene-by-gene through the rest. Counting `?` as a value would make whole breeds' gene sets read as spuriously rare, so the exclusion rule is load-bearing rather than a tidy-up.
...
- **Minimum sample.** A locus with fewer than `minKnown` **alleles** has no meaningful rare/common distinction. Default **4** (= 2 pets with a known reading); such loci render as "missing data", same as `?`. `minKnown` only suppresses the *degenerate* case; it is **not** where the ramp becomes informative.
  ...
  **In the two v1 (local) tiers this rule rarely fires.** Reveal is a property of the *player*, not the animal, so every pet in your own collection should be unknown at the same loci: a locus is then either known across your whole collection or unknown across all of it, and the denominator is `N` or `0` — never partial. Partial denominators arise once a population mixes players at different reveal levels, i.e. the deferred community tier, which is where `minKnown` actually earns its keep. (This follows from the skill-gating model — see the `project-genetics-skill-unknown-genes` note — not from the measurement: the catalogue holds one partial genome, so cross-pet uniformity within a single player is **not** directly verified. If it turned out to be false, `minKnown` becomes live for local tiers too, which the design already handles.) With self-inclusion, a pure pet contributes 2 of its locus's alleles, so at 2 pets its own allele is ≥50% → always "common". The ramp gets meaningful around ~5+ pets of a species (see §7) — sooner than genotype counting allowed, because the allele denominator is twice as large and a singleton allele reads at half the frequency.
...
- **Populations will genuinely mix reveal levels.** The catalogue spans three characters, one of whom is at a lower Genetics skill — so unlike the local tiers, community baselines will have per-locus denominators that vary by hundreds of loci. Measured on this catalogue the distortion is small (including the partial genome shifts `p_D` by a median of 0.0028, p95 0.0179 over 1576 loci, and only drops the denominator from 52 to 51), but that is with one partial genome in 52. A catalogue with many low-skill uploaders would concentrate the loss on exactly the high-unlock chromosomes, which is the case `minKnown` exists for.
...
- **Breed is not a confound, and the baseline must stay species-scoped.** Every horse carries every breed's genes — the horse gene set is 256 untagged loci plus **132 loci for each of 10 breeds** (Satincoat, Statehelm, Calico, Standardbred, Paint, Kurbone, Ilmarian, Blanketed, Leopard, Plateau Pony) = the full 1576. Breed does not determine which loci a horse *has*, only which ones are *expressed*. So a breed's allele frequencies are measured across the whole collection, and **the lens reports how rare Calico genes are even for a player who has never owned a Calico** — which is a genuine capability, not a workaround: it tells you whether you are already sitting on scarce material for a breed you might acquire later.
  ...
  **The capability is gated by reveal, not by ownership.** The measured partial genome in §2 has *all 132 Calico loci unknown* — so for that player the Calico baseline does not exist, no matter how many horses they own, and those cells correctly render as missing data rather than as rare. The cross-breed reading is therefore available per breed, exactly as far as the player's revealed loci reach. The legend's "across N pets" figure is per-baseline and does not convey this; a per-breed coverage hint is a plausible v2 addition (§10).
...
- **Missing data must be synthesised, and the fixture shape is now known.** The calibration collection has **zero** `?` genotypes (`unknown_genes = 0` across all 37 pets — the author's Genetics skill reveals everything), so the `?` path cannot be validated against local data. Fixtures should mimic the measured shape rather than sprinkling `?` at random: **~33% of loci unknown, one or two breeds' gene sets almost entirely missing, and the remainder interleaved gene-by-gene within blocks** (§2). The interleaving is the part a naive fixture gets wrong, and it is what proves cells are resolved individually rather than by row or region. Also cover the below-`minKnown` locus, which is the one that does *not* get `gene-unknown` for free (§4), and a mixed-reveal population where the denominator varies per locus (community-tier shape).
