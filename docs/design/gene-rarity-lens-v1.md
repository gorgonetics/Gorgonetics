...
- **`Unknown` (`?`) is missing data, never a value.** In Project Gorgon a locus reads `?` only because the player's Genetics skill hasn't revealed it yet — uniformly across the whole collection, and biased toward loci that unlock at the highest skill levels. Counting `?` would make late-unlock loci look artificially rare. So `?` contributes **no alleles and no denominator**, and a pet's own `?` cell has no rarity signal. See the `project-genetics-skill-unknown-genes` note.
  ...
  **Confirmed against the community catalogue** (52 shared Horse genomes): reveal is *bimodal and chromosome-clustered*, not scattered. Of 52 genomes, **51 are fully revealed and 1 is missing 522 of 1576 loci (33%)** — and those 522 are whole chromosomes, not a spray: chr07, 45, 46, 47 and 48 are **100%** unknown, chr32 97.6%, chr30 96.4%, chr29 95.0%. This is the design's premise measured rather than assumed, and it makes the exclusion rule load-bearing: counting `?` as a value would not gently bias the ramp, it would render entire chromosomes as spuriously rare. The unknowns fall on breed-tagged and untagged loci in proportion to the genome (399 / 123 against a 1320 / 256 split), so the reveal order is chromosomal and has nothing to do with breed.
...
- **Minimum sample.** A locus with fewer than `minKnown` **alleles** has no meaningful rare/common distinction. Default **4** (= 2 pets with a known reading); such loci render as "missing data", same as `?`. `minKnown` only suppresses the *degenerate* case; it is **not** where the ramp becomes informative.
  ...
  **In the two v1 (local) tiers this rule almost never fires, by construction.** Reveal is a property of the *player's* Genetics skill, so every pet in your own collection is unknown at the same loci: a locus is either known across your whole collection or unknown across all of it, and the denominator is `N` or `0` — never partial. The visible consequence is that missing-data cells appear as **whole chromosome bands**, not scattered specks, which is far easier to explain ("your Genetics skill hasn't unlocked these") than a peppering of dead cells. Partial denominators only arise once a population mixes players at different skill levels — i.e. the deferred community tier, which is where `minKnown` will actually earn its keep. (Inference from the skill-gating model plus the chromosome clustering measured above; the catalogue holds only one partial genome, so cross-pet uniformity within a single player is not directly verified.) With self-inclusion, a pure pet contributes 2 of its locus's alleles, so at 2 pets its own allele is ≥50% → always "common". The ramp gets meaningful around ~5+ pets of a species (see §7) — sooner than genotype counting allowed, because the allele denominator is twice as large and a singleton allele reads at half the frequency.
...
- **Missing data must be synthesised, and the fixture shape is now known.** The calibration collection has **zero** `?` genotypes (`unknown_genes = 0` across all 37 pets — the author's Genetics skill reveals everything), so the `?` path cannot be validated against local data. Fixtures should mimic the measured community shape rather than sprinkling `?` at random: **whole chromosomes unknown** (~33% of loci, entire chromosome ranges at 100%), since that is what real partial reveals look like and it exercises the "whole band renders as missing data" case. Also cover the below-`minKnown` locus, which is the one that does *not* get `gene-unknown` for free (§4), and a mixed-reveal population where the denominator varies per locus (community-tier shape).
